...
export default function DropdownTabs({ onSelectCategory }) {
  ...
  return (
    <div className="relative w-full block md:hidden">
      <div
        className="w-full h-[40px] border border-[#DFE1E7] rounded-lg shadow-custom flex items-center justify-between px-3 cursor-pointer"
        onClick={toggleDropdown}
      >
        <h2 className="text-[#818898] text-[16px] leading-[25px] font-normal">{selectedCategory}</h2>
        <IoIosArrowDown className={`text-[#818898] text-[20px] transition-transform ${isOpen ? 'rotate-180' : ''}`} />
      </div>
      {isOpen && (
        <div className="py-3 absolute top-[100%] left-0 mt-2 w-full border border-[#DFE1E7] rounded-lg bg-white shadow-custom z-10">
          {categories.map((category) => (
            <div
              key={category.name}
              className="flex items-center px-3 py-2 text-[#818898] hover:bg-[#F0F0F0] cursor-pointer"
              onClick={() => handleOptionClick(category)}
            >
              <Image src={category.image} alt={category.name} width={20} height={20} />
              <h3 className="text-[16px] leading-[25.6px] font-normal ml-3">{category.name}</h3>
            </div>
          ))}
        </div>
      )}
