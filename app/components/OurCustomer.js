        ...
        <div className='bg-[#F8FAFB] w-full py-20 px-5'>
            <div className='w-full'>
                <div className='flex flex-col text-center gap-4'>
                    <div className="shadow-custom border border-[#DFE1E7] bg-white rounded-[1000px] mx-auto px-3 py-1 max-w-[141px] flex items-center gap-1.5">
                    ...
                    <div className='w-full max-w-[1160px] mx-auto'>
                        <div className='w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
                            {visibleCards.map((card, index) => (
                                <div key={index} className='bg-white px-3.5 py-5 lg:p-6 rounded-[16px]'>
                                    <div className='flex items-center gap-2'>
                                        <Image src='/assets/icons/star.svg' alt='star' width={108} height={20} />
                                        <span className='text-[#666D80] text-[16px] leading-[25px] font-medium mt-1'>{card.rating}</span>
                                    </div>
                                    <p className='mt-4 text-[#0D0D12] text-[18px] font-medium leading-[27px]'>{card.review}</p>
                                    <div className='mt-10 flex items-center gap-3'>
                                        <Image src={card.profileImg} alt='profile' width={48} height={48} />
                                        <div>
                                            <h3 className='text-[#0E1012] font-medium text-[16px] leading-[25px]'>{card.name}</h3>
                                            <p className='text-[#566171] text-[14px] font-normal leading-[21px]'>{card.position}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
                        ...
                        <div className='w-full sm:w-fit absolute z-40 left-1/2 -bottom-5 transform -translate-x-1/2'>
                            {!showAll ? (
                                <button
                                    ...
                                    className='bg-[#5F57FF] shadow-custom-one sm:w-fit w-full  px-4 h-[52px] rounded-[1000px] text-[16px] leading-[24px] font-semibold text-white transition transform  hover:shadow-lg '
                                >
